...
conversation_jd = get_conversation(template_var.template_jd)

response = conversation_jd.invoke({"job_description_text": job_description})

job_description = response.content

print(job_description)

conversation_resume = get_conversation(template_var.template_resume)
...
for i in range(len(resume_df)):
    ...
    response = conversation_resume.invoke({"resume_text": resume_text})
...
conversation_score = get_conversation(template_var.template_score)

for i in range(len(resume_df)):
    response = conversation_score.invoke({"resume_text": resume_df['resume_file_text'][i], "job_description": job_description})
